#!/usr/bin/env python3
"""Builds docs/index.html: the whole record of Bluff Show in one file.
...
Reads:
    ~/.claude/knowledge/projects/bluff-show/*.md and log/*.md   (override: KNOWLEDGE=...)
...
NAME = "Bluff Show"
ARTIFACT = "https://claude.ai/artifact/UpGVde7PPbjiAwg9w4Q4kb"   # the private link the --artifact copy is published to

# The picture at the top: (path from the project root, alt text).
HERO = ("test/shots/desktop-01-home.png",
        "The Bluff Show home screen: a pink marquee with the gold title and the line Lie well, spot the truth, "
        "above three chunky buttons to play on this device, host a room or join a room.")
...
GALLERIES = [
    ("pass-the-device", "A pass-the-device game, on a laptop",
     "One game in order, from setup to the podium, as the browser tests play it at 1440 x 900. "
     "The players are the test's Luqman, Aina, Hafiz and Siti, and the lies are the test's "
     "placeholder text, so they read like \"Lie by Siti 1\".",
     "wide", [
         ("test/shots/desktop-02-setup.png",
          "Setup: four players named, five rounds chosen, and the categories showing how many of the 151 questions each holds."),
         ("test/shots/desktop-03-custom.png",
          "Players can add questions about people in the room; this one is by Siti, so she will sit its round out."),
         ("test/shots/desktop-04-board-write.png",
          "Round 1 opens on Siti's custom question, with Siti greyed out as its author while the other three write."),
         ("test/shots/desktop-05-pass.png",
          "Before every secret turn the device asks to be handed to one player, and everyone else is told to look away."),
         ("test/shots/desktop-06-write.png",
          "Luqman writes his lie; the red warning is left from a moment earlier, when the real answer was typed and refused."),
         ("test/shots/desktop-07-board-pick.png",
          "Round 2's answers board mixes the true answer with the players' lies, lettered A to D, before the device goes round for picks."),
         ("test/shots/desktop-08-pick.png",
          "Luqman picks in private and is not offered his own lie, so he sees three answers instead of four."),
         ("test/shots/desktop-10-reveal-truth.png",
          "Nobody fell for a lie this round, so the reveal goes straight to the truth: who found it, +1000 each, and the fact behind it."),
         ("test/shots/desktop-11-scores.png",
          "The scoreboard after round 2 ranks everyone with a bar and shows what each player gained that round."),
         ("test/shots/desktop-12-final.png",
          "The final screen: a podium for the top three, the Master Liar and Truth Detective awards, and fourth place below."),
     ]),
    ("room-tv", "Room mode: the TV",
     "The host screen at 1920 x 1080. It runs the rules and shows the game to the room; nobody "
     "types on it. The tests play this with one TV tab and three phone tabs talking over "
     "BroadcastChannel, because the Supabase project is not created yet.",
     "wide", [
         ("test/shots/room-tv-lobby.png",
          "The TV opens a room with a four-letter code and a QR code; three contestants have joined from their phones (the test's address is localhost)."),
         ("test/shots/room-tv-write.png",
          "Round 2 on the TV: the question, the writing timer, ticks for Aina and Hafiz who have sent lies, and the code for anyone late."),
         ("test/shots/room-tv-pick.png",
          "The pick phase shows every answer large with a 45-second timer while each player chooses on their own phone."),
         ("test/shots/room-tv-reveal.png",
          "The reveal puts each answer up on its own; here round 1's first of two answers has just appeared."),
         ("test/shots/room-tv-final.png",
          "The winner screen after five rounds, with Aina on top of the podium and taking both awards."),
     ]),
    ("room-phones", "Room mode: a player's phone",
     "The same room game on Aina's phone. A phone only shows what the host sends it and sends back "
     "her moves, so it never knows the truth before the reveal.",
     "tall", [
         ("test/shots/room-phone-lobby.png",
          "After joining, the phone confirms three players are in, and as the first to join Aina gets the start button."),
         ("test/shots/room-phone-write.png",
          "In round 1 the real answer was refused, then Need an idea? filled in a decoy lie sent by the host."),
         ("test/shots/room-phone-pick.png",
          "In round 2's pick the phone shows three answers, not the TV's four, because Aina's own lie is hidden from her."),
         ("test/shots/room-phone-final.png",
          "At the end the phone tells Aina she won, with the same 10000 points the TV shows."),
     ]),
    ("phone", "A pass-the-device game, on a phone",
     "The pass-the-device game again at iPhone size in portrait, where the game is most often "
     "played. Different questions from the laptop run, because each run draws its own.",
     "tall", [
         ("test/shots/phone-01-home.png",
          "The home screen stacks the three ways to play; the note says room mode still needs config.js filled in."),
         ("test/shots/phone-04-board-write.png",
          "The round board wraps the player chips onto two rows; Siti sits out the question she wrote."),
         ("test/shots/phone-05-pass.png",
          "The hand-over screen, with the big button within thumb reach."),
         ("test/shots/phone-08-pick.png",
          "Luqman's pick leaves out his own lie, and his choice turns pink before he locks it in."),
         ("test/shots/phone-09-reveal-lie.png",
          "A lie is revealed with who fell for it, who wrote it, and the +500 the writer earns."),
         ("test/shots/phone-10-reveal-truth.png",
          "The truth is stamped last, with who found it and the one-line fact about the cotton candy machine."),
         ("test/shots/phone-11-scores.png",
          "After round 2 Luqman leads on 3000, and Siti's empty bar shows she has not scored yet."),
         ("test/shots/phone-12-final.png",
          "The final podium on a phone, with Luqman taking first place and both awards."),
     ]),
    ("sideways", "A phone held sideways",
     "Phone landscape, 844 x 390 points. The tests check nothing scrolls sideways; these show "
     "what fits on the first screen.",
     "wide", [
         ("test/shots/phone-landscape-07-board-pick.png",
          "Held sideways, the answers board puts two answers per row, and the second row starts at the bottom edge."),
         ("test/shots/phone-landscape-09-reveal-lie.png",
          "The lie reveal leaves who fell for it below the first screen, so players scroll down to see it."),
     ]),
    ("tablet", "A tablet",
     "Tablet portrait, where the whole setup screen fits and the reveal has room to breathe.",
     "tall", [
         ("test/shots/tablet-02-setup.png",
          "The full setup screen, including the panel for the players' own questions and the sound switch."),
         ("test/shots/tablet-09-reveal-lie.png",
          "A lie reveal: Aina fell for Siti's lie about Velcro, which earns Siti +500."),
     ]),
]
...
PIPELINE_LEDE = ("Bluff Show is a static web page with no build step. One rules file drives both ways "
                 "to play, every built-in fact was researched with a source, and each change is proved "
                 "by rules tests and by full games played in headless Chrome, which also take the "
                 "screenshots on this page.")

# (step, what happens, where it lives)
PIPELINE = [
    ("Idea and scoping",
     "Luqman chose Bluff Trivia over Imposter, Word Bomb and Forehead Guess, with both ways to play, for friends and family, in a retro TV show look.",
     "01-idea.md · D-001"),
    ("Rules engine",
     "One game state moves through write, pick, reveal and scores each round, then final. Scoring and the too-close-to-the-truth check live here, with no screen code.",
     "engine.js"),
    ("Sourced question bank",
     "Six research agents, one per category, checked every fact by web search and recorded a source URL; myths were dropped. 152 written, one duplicate removed.",
     "questions.js · D-004"),
    ("Screens and look",
     "Every screen for the three ways to use a device (pass-the-device, host, phone), styled as a 70s quiz show and sized up for TV screens.",
     "index.html · app.js"),
    ("Room mode",
     "The host screen runs the rules and broadcasts a public view that hides the truth; phones send moves. Messages go over Supabase Realtime, or BroadcastChannel for tests.",
     "room.js · config.js · D-002, D-003"),
    ("Rules tests",
     "Checks the rules on their own: answer matching, custom questions, sitting out, scoring and awards.",
     "test/engine.test.js"),
    ("Browser tests and screenshots",
     "Plays whole games through the real screens in headless Chrome at four sizes, then a room game with a TV tab and three phone tabs, saving screenshots on the way.",
     "test/run.sh · test/e2e.js · test/cdp.js"),
    ("Room mode live (next)",
     "Create the Supabase project in the dashboard, fill in config.js, publish on GitHub Pages and play a real game night on phones.",
     "03-milestones.md · M2, M3"),
    ("Project record",
     "This page, rebuilt from the notes, the screenshots and the git log whenever the game changes.",
     "tools/docs/build_docs.py"),
]

# (tool, what it does here)
TOOLS = [
    ("HTML, CSS and JavaScript", "The whole game, with no framework and no build step; pass-the-device mode even runs straight from the file."),
    ("Supabase Realtime", "Broadcast channels carry room messages between the TV and the phones. No tables; supabase-js 2.116.0 is loaded only when a room opens."),
    ("qrcode-generator 2.0.4", "Draws the QR code on the TV so phones can join by scanning."),
    ("BroadcastChannel", "Browser tabs talk to each other with `?transport=local`, so room mode can be tested with no internet."),
    ("Browser APIs", "Web Audio for sound, the Wake Lock API to keep the host screen on, and localStorage for settings and a phone's player ID."),
    ("Google Fonts", "Bungee and Bungee Shade for the marquee lettering, Rubik for everything else."),
    ("Research agents with web search", "Wrote and checked the question bank, one agent per category, each fact with its source."),
    ("Node", "Runs the rules tests and drives the browser tests."),
    ("Google Chrome, headless", "Plays the game for the tests through a small DevTools Protocol driver with no dependencies, and takes the screenshots."),
    ("Python", "`python3 -m http.server` serves the game for the tests, and builds this record."),
]
...
CATALOGUES = [
    ("tests", "Tests",
     "Run everything with ./test/run.sh: the rules tests first, then the browser games. Needs Node and Google Chrome.",
     ["test/run.sh", "test/*.js"]),
    ("source", "The game's source files",
     "The game itself, apart from index.html, which holds every screen and the whole look.",
     ["*.js"]),
]
...
def counts():
    ...
    questions = len(re.findall(r'^\s*\{"cat":', (PROJECT / "questions.js").read_text(), re.M))
    cats = re.search(r"const CATS = \[(.*?)\]", (PROJECT / "app.js").read_text())
    categories = len(re.findall(r"'[^']+'", cats.group(1))) if cats else 0
    checks = len(re.findall(r"^\s*ok\(", (PROJECT / "test/engine.test.js").read_text(), re.M))
    return [
        ("Questions", str(questions)),
        ("Categories", str(categories)),
        ("Rules checks", str(checks)),
...
FONTS = "https://fonts.googleapis.com/css2?family=Bungee&family=Rubik:ital,wght@0,400;0,500;0,600;1,400&family=Space+Mono&display=swap"
